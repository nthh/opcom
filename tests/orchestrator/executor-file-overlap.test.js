...
class MockSessionManager {
    ...
    getSession(id) { return this.knownSessions.get(id); }
    registerSession(session) { this.knownSessions.set(session.id, session); }
    on(event, handler) {
...
// --- Hoisted mocks ---
const { mockQueryGraphContext, mockScanTickets } = vitest_1.vi.hoisted(() => ({
    mockQueryGraphContext: vitest_1.vi.fn(),
...
(0, vitest_1.describe)("Executor file-overlap scheduling", () => {
    let mockSM;
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("serializes steps that share overlapping files", async () => {
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t1")
                return { relatedFiles: ["src/foo.ts", "src/bar.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: ["src/foo.ts", "src/baz.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
        ...
        (0, vitest_1.expect)(plan.steps.filter((s) => s.status === "in-progress")).toHaveLength(1);
        (0, vitest_1.expect)(plan.steps.filter((s) => s.status === "ready")).toHaveLength(1);
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts steps with disjoint file sets in parallel", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t1")
                return { relatedFiles: ["src/alpha.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: ["src/beta.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
            { ticketId: "t1", projectId: "p", status: "ready", blockedBy: [] },
        ...
        (0, vitest_1.expect)(mockSM.startCalls).toHaveLength(2);
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts higher-priority step first when files overlap", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t-low")
                return { relatedFiles: ["src/shared.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-high")
                return { relatedFiles: ["src/shared.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        ...
        (0, vitest_1.expect)(mockSM.startCalls).toHaveLength(1);
        (0, vitest_1.expect)(mockSM.startCalls[0].ticketId).toBe("t-high");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("breaks priority ties by fewer blockedBy dependencies", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t-many-deps")
                return { relatedFiles: ["src/overlap.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-few-deps")
                return { relatedFiles: ["src/overlap.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
        ...
        (0, vitest_1.expect)(mockSM.startCalls).toHaveLength(1);
        (0, vitest_1.expect)(mockSM.startCalls[0].ticketId).toBe("t-few-deps");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts all ready steps when graph is unavailable (queryGraphContext returns null)", async () => {
        ...
        (0, vitest_1.expect)(mockSM.startCalls).toHaveLength(3);
        (0, vitest_1.expect)(plan.steps.filter((s) => s.status === "in-progress")).toHaveLength(3);
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts all ready steps when queryGraphContext throws", async () => {
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("holds back a ready step that overlaps with an in-progress step", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t-active")
                return { relatedFiles: ["src/model.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-waiting")
                return { relatedFiles: ["src/model.ts", "src/controller.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-free")
                return { relatedFiles: ["src/utils.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
        ...
        (0, vitest_1.expect)(startedTickets).toContain("t-free");
        (0, vitest_1.expect)(startedTickets).not.toContain("t-waiting");
        // t-active was already in-progress, so it should NOT appear in startCalls
        (0, vitest_1.expect)(startedTickets).not.toContain("t-active");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("only starts one of two ready steps that overlap with each other (no active overlap)", async () => {
        mockScanTickets.mockResolvedValue([
            { id: "t1", title: "T1", status: "open", priority: 2, type: "feature", filePath: "/tmp/t1.md", deps: [], links: [], tags: {} },
            { id: "t2", title: "T2", status: "open", priority: 2, type: "feature", filePath: "/tmp/t2.md", deps: [], links: [], tags: {} },
        ]);
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t1")
                return { relatedFiles: ["src/shared.ts", "src/a.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: ["src/shared.ts", "src/b.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
            { ticketId: "t1", projectId: "p", status: "ready", blockedBy: [] },
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts held-back step after the blocking step completes", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        (0, vitest_1.expect)(mockSM.startCalls).toHaveLength(2);
        (0, vitest_1.expect)(mockSM.startCalls[1].ticketId).toBe("t2");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("treats steps with empty relatedFiles as non-overlapping", async () => {
        mockScanTickets.mockResolvedValue([
            { id: "t1", title: "T1", status: "open", priority: 2, type: "feature", filePath: "/tmp/t1.md", deps: [], links: [], tags: {} },
            { id: "t2", title: "T2", status: "open", priority: 2, type: "feature", filePath: "/tmp/t2.md", deps: [], links: [], tags: {} },
        ]);
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            ...
            if (ticketId === "t1")
                return { relatedFiles: [], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: [], testFiles: [], driftSignals: [] };
            return null;
        });
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("starts the correct subset when some steps overlap and others do not", async () => {
        mockScanTickets.mockResolvedValue([
            { id: "t1", title: "T1", status: "open", priority: 1, type: "feature", filePath: "/tmp/t1.md", deps: [], links: [], tags: {} },
            { id: "t2", title: "T2", status: "open", priority: 2, type: "feature", filePath: "/tmp/t2.md", deps: [], links: [], tags: {} },
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t1")
                return { relatedFiles: ["src/a.ts", "src/shared.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: ["src/shared.ts", "src/b.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t3")
                return { relatedFiles: ["src/c.ts", "src/d.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
            { ticketId: "t1", projectId: "p", status: "ready", blockedBy: [] },
        ...
        const startedTickets = mockSM.startCalls.map((c) => c.ticketId);
        (0, vitest_1.expect)(startedTickets).toHaveLength(2);
        (0, vitest_1.expect)(startedTickets).toContain("t1");
        (0, vitest_1.expect)(startedTickets).toContain("t3");
        (0, vitest_1.expect)(startedTickets).not.toContain("t2");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("holds back a ready step that overlaps with a verifying step", async () => {
        mockScanTickets.mockResolvedValue([
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            if (ticketId === "t-verifying")
                return { relatedFiles: ["src/api.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-ready")
                return { relatedFiles: ["src/api.ts", "src/handler.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t-nonoverlap")
                return { relatedFiles: ["src/utils.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        ...
        const startedTickets = mockSM.startCalls.map((c) => c.ticketId);
        (0, vitest_1.expect)(startedTickets).toContain("t-nonoverlap");
        (0, vitest_1.expect)(startedTickets).not.toContain("t-ready");
        (0, vitest_1.expect)(startedTickets).not.toContain("t-verifying");
        executor.stop();
        await runPromise;
    });
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("respects maxConcurrentAgents even when no file overlaps exist", async () => {
        mockScanTickets.mockResolvedValue([
            { id: "t1", title: "T1", status: "open", priority: 2, type: "feature", filePath: "/tmp/t1.md", deps: [], links: [], tags: {} },
            { id: "t2", title: "T2", status: "open", priority: 2, type: "feature", filePath: "/tmp/t2.md", deps: [], links: [], tags: {} },
        ...
        mockQueryGraphContext.mockImplementation((_proj, ticketId) => {
            ...
            if (ticketId === "t1")
                return { relatedFiles: ["src/a.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t2")
                return { relatedFiles: ["src/b.ts"], testFiles: [], driftSignals: [] };
            if (ticketId === "t3")
                return { relatedFiles: ["src/c.ts"], testFiles: [], driftSignals: [] };
            return null;
        });
        const plan = makePlan([
            { ticketId: "t1", projectId: "p", status: "ready", blockedBy: [] },
    ...
    // -----------------------------------------------------------------------
    (0, vitest_1.it)("caches file lists per ticketId (queryGraphContext called once per step)", async () => {
        mockScanTickets.mockResolvedValue([
            { id: "t1", title: "T1", status: "open", priority: 2, type: "feature", filePath: "/tmp/t1.md", deps: [], links: [], tags: {} },
        ]);
        ...
        const t1Calls = mockQueryGraphContext.mock.calls.filter((c) => c[1] === "t1");
        (0, vitest_1.expect)(t1Calls).toHaveLength(1);
        executor.stop();
        await runPromise;
    });
});
//# sourceMappingURL=executor-file-overlap.test.js.map
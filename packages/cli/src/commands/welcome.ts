...
export async function runWelcome(): Promise<void> {
  ...
  try {
    ...
    console.log("");
    console.log(`  ${BOLD}2. Plan${RESET}`);
    console.log(`    Group tickets into execution plans with dependencies.`);
    console.log(`    ${CYAN}opcom plan create${RESET} → ${CYAN}opcom plan show${RESET}`);
    console.log("");
    console.log(`  ${BOLD}3. Run the TUI${RESET}`);
    console.log(`    ${CYAN}npx opcom tui${RESET} to launch, manage, and monitor agents.`);
    console.log(`    The TUI is where agents are started, paused, and reviewed.`);
    console.log("");
